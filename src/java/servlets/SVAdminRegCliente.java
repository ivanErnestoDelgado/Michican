 ...
 * @author SADValenz
 */
@WebServlet(name = "SVAdminRegCliente", urlPatterns = {"/SVAdminRegCliente"})
public class SVAdminRegCliente extends HttpServlet {
    ...
    HttpSession sesion=request.getSession();
        ...
        if(!credencialesSonValidas(correo, contrasena, telefono, nombre, apellido,sexo)){
            response.sendRedirect("AdminCliente.jsp?mensaje=Datos_ingresados_invalidos");
            return;
        }
        ...
        response.sendRedirect("AdminCliente.jsp?mensaje=Cliente_insertado_exitosamente");
        return;
        }else{
        response.sendRedirect("AdminCliente.jsp?mensaje=El_correo_ya_esta_utilizado");
        return;
        }
